...
For slow walk we had a value of `20`.  So if we divide 20/450 we get `.044`.  So this will be our scalar for slow walk.  Go to **InputAction Slow Walk** and replace the hard coded values with a **Set Sprint Multiplier** with a value of `0.044` when the key is pressed and a value of `1.0` when it is released.
